...
public class QRCodeLidoUseCase : IQRCodeLidoUseCase
{
    ...
    private async Task Validar(Domain.Entidades.Codigos codigo, Domain.Entidades.Usuario usuarioLogado)
    {
        if (codigo is null)
        {
            throw new AdminSiteException(ResourceMensagensDeErro.CODIGO_NAO_ENCONTRADO);
        }

        if (codigo.UsuarioId == usuarioLogado.Id)
        {
            throw new AdminSiteException(ResourceMensagensDeErro.VOCE_NAO_PODE_EXECUTAR_ESTA_OPERACAO);
        }
        ...
        if (existeConexao)
        {
            throw new AdminSiteException(ResourceMensagensDeErro.ESTA_CONEXAO_JA_EXISTE);
        }
    }
}
